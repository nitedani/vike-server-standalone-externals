...
/**
 * Log a debug object with a message if debugging is enabled
 * @param message - The message to log
 * @param obj - The object to log
 */
function debugObj(message: string, obj: any): void {
  if (DEBUG) {
    try {
      ...
      console.log(DEBUG_PREFIX, message, JSON.parse(str));
    } catch (err) {
      console.log(DEBUG_PREFIX, message, "Object can't be serialized:", obj);
    }
  }
}
...
export function standaloneExternals(): Plugin {
  ...
  return {
    ...
    config(config) {
      ...
      debug("Vike config server standalone:", standaloneConfig);

      // Only apply externals if standalone config is present
      if (!standaloneConfig) {
        debug("No standalone config found, skipping");
        return;
      }
      ...
      assert(
        Array.isArray(configExternals),
        "Config externals must be an array"
      );

      debug("Config externals:", configExternals);
      debug("Default externals:", DEFAULT_EXTERNALS);
      debug("Combined externals:", [...DEFAULT_EXTERNALS, ...configExternals]);
    ...
    closeBundle: {
      ...
      async handler() {
        ...
        assert(
          ...
          "Build config must include root and outDir"
        );
        ...
        if (standaloneFiles.length === 0) {
          console.log(
            "No standalone files found, skipping dependency processing."
          );
          return;
        }
...
async function processStandalone(
  ...
  assert(
    packageInfo?.versionCounts instanceof Map,
    "Version counts must be a Map"
  );
  assert(
    packageInfo?.workspacePackages instanceof Map,
    "Workspace packages must be a Map"
  );
...
async function traceDependencies(
  ...
  assert(
    result?.fileList && result?.reasons,
    "File tracing must return fileList and reasons"
  );
...
function buildDependencyGraph(
  ...
  debug(
    `${imports.size} files have imports, ${importedBy.size} files are imported`
  );
...
async function analyzePackages(
  ...
  for (const [name, locations] of packageLocations.entries()) {
    ...
    debug(
      `Package ${name} has ${locations.size} versions at: ${[...locations].join(
        ", "
      )}`
    );
  }
...
async function copyDependencies(
  ...
  const normalizedOutDir = path.normalize(
    path.isAbsolute(outDirAbs) ? outDirAbs : path.resolve(outDirAbs)
  );
  ...
  for (const [file, destPath] of relocatedFiles) {
    debug(
      `Adjusting relative imports for relocated file: ${file} -> ${destPath}`
    );
...
function adjustPathsForRelativeImports(
  ...
  for (const importedFile of importedFiles) {
    ...
    debug(
      `Is relative import: ${
        relPathFromSource.startsWith("..") || relPathFromSource.startsWith(".")
      }`
    );
    ...
    if (
      ...
      debug(
        `New destination for import (before safety check): ${newImportDest}`
      );
      ...
      debug(
        `New destination for import (after safety check): ${newImportDest}`
      );
      ...
      if (currentDest && !currentDest.includes("/node_modules/")) {
        ...
        debug(
          `Not updating destination for ${importedFile} - already has a node_modules destination or no destination`
        );
      }
    } else {
...
function mapFilePath(
  ...
  if (file.includes("node_modules/")) {
    ...
    debug(
      `Path parts after splitting by node_modules/: ${JSON.stringify(parts)}`
    );
    ...
    if (packageName) {
      ...
      if (versionCount === 1) {
        ...
        debug(
          `Package ${packageName} has multiple versions (${versionCount}), not hoisting`
        );
      }
    } else {
  ...
  if (workspaceMatch) {
    assert(
      ...
      "Workspace match must have package name"
    );
    assert(
      typeof workspaceMatch.packageName === "string",
      "Package name must be a string"
    );
    assert(
      typeof workspaceMatch.relativePath === "string",
 ...
 */
function ensurePathIsWithinDirectory(
  filePath: string,
  ...
  assert(
    typeof containingDir === "string",
    "Containing directory must be a string"
  );
  ...
  const absContainingDir = path.isAbsolute(containingDir)
    ? containingDir
    : path.resolve(containingDir);
  ...
  const isPathWithinDir =
    normalizedPath.startsWith(dirWithSep) || normalizedPath === normalizedDir;
...
function findWorkspaceMatch(
  ...
  debug(
    `Sorted workspace directories (first 5): ${dirs.slice(0, 5).join(", ")}${
      dirs.length > 5 ? "..." : ""
    }`
  );
...
function toPosixPath(path: string): string {
  debug(`toPosixPath() called for "${path}"`);
...
function assertUsage(condition: unknown, message: string): asserts condition {
  ...
  const errorMessage = `[vike-server-externals][Wrong Usage] ${message}`;
  debug(`USAGE ASSERTION FAILED: ${errorMessage}`);
